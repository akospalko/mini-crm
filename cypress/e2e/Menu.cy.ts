...
describe("Menu test: open (create, edit), close, render content ", () => {
  ...
  const checkAndClickEditButton = (
    testIDButton: string,
    testIDManagementItem: string,
    dataAction: string
  ) => {
    ...
    cy.get("@managementItem").then((element) => {
      ...
      if (isItemDataAvailable) {
        cy.get(
          `[data-testid=${testIDButton}][data-action='${dataAction}']:first`
        ).should("exist")
        cy.get(
          `[data-testid=${testIDButton}][data-action='${dataAction}']:first`
        ).click()
    ...
    switch (access) {
      case "create":
        activeOpenMenuButton = () =>
          checkAndClickCreateButton("button-menu-open")
        break
      case "edit-client":
        activeOpenMenuButton = () =>
          checkAndClickEditButton(
            "button-manage-item",
            "management-client-item",
            "data-action-edit"
          )
        break
      case "edit-property":
  ...
  it("opens and closes the menu modal on another route", () => {
    openAndCloseMenuModal(
      "http://localhost:5173/manage-properties",
      "edit-property"
    )
  })
})
