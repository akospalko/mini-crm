...
const useFormHandlers = () => {
  ...
  const {clients, activeClient, dispatch: dispatchClients, REDUCER_ACTIONS_CLIENT} = useClients();
  const {property, activeProperty, dispatch: dispatchProperty, REDUCER_ACTIONS_PROPERTY} = useProperty();
  ...
  const createNewClient = (): void => {
    ...
    const clientFormValues = Object.entries(formData)
      .filter(([key]) => Object.values(ClientFormDataE).includes(key as ClientFormDataE))
      .map(([key, value]) => {
  ...
  const updateExistingProperty = (): void => {
    ...
    const updatedProperties: PropertyItemI[] = updateDataArray(property, updatedProperty) as PropertyItemI[];
