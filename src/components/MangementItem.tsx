...
import {ACTIVE_MANAGEMENT, ACTIVE_MENU_ACTION_TYPE, DATABASE_RESOURCES} from "../types/actionTypes";
import {ClientItemI, PopulateFormDataWithActiveClientI, PropertyItemI} from "../types/types";
...
const MangementItem = ({itemData, activeManagement}: ClientManagementI) => {
  ...
  const deleteItemHandler = (isClientAction: boolean): void => {
    ...
    if (isClientAction === undefined || isClientAction === null || typeof isClientAction !== 'boolean') {
      throw new Error(`${text["error-invalid-argument"]} ${isClientAction}`);
    }

    // delete item from array
    const updatedItemsArray: ClientItemI[] | PropertyItemI[] = deleteItemByID(itemData.id, isClientAction ? clients : property);
    ...
    localStorage.setItem(
      isClientAction ? DATABASE_RESOURCES.CLIENTS : DATABASE_RESOURCES.PROPERTIES, 
  ...
  const editItemHandler = (isClientAction: boolean) => {
    ...
    menuContentChangeHandler(isClientAction ? ACTIVE_MENU_ACTION_TYPE.EDIT_CLIENT : ACTIVE_MENU_ACTION_TYPE.EDIT_PROPERTY);

    // find activec item
    const activeItem: ClientItemI | PropertyItemI = findActiveArrayItem(itemData.id, isClientAction ? clients : property);
    ...
    const activeItemProcessed: PopulateFormDataWithActiveClientI |PropertyItemI = isClientAction ? getActiveClientWithRemappedProperties(activeItem as ClientItemI) : activeItem as  PropertyItemI;
  ...
  return (
    <div 
      ...
      className="grid grid-cols-[150px,1fr,40px,40px] lg:grid-cols-[200px,1fr,40px,40px] grid-rows-1 h-[56px] grid-flow-col overflow-hidden bg-color_2 focus-within:outline-2 rounded p-[0.5rem] gap-[0.5rem]"
    > 
      {itemContent}
