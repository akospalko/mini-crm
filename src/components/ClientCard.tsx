...
const ClientCard = ({ clientData }: ClientCardPropsI) => {
  ...
  // MEMO
  const convertedPropertyData: Record<string, PropertyItemRemappedT> =
    useMemo(() => {
      return property?.reduce(
        (
          accumulator: Record<string, PropertyItemRemappedT>,
          { id, ...rest }
        ) => {
          accumulator[id] = {
            ...rest,
            type: rest.type as unknown as PropertyTypeE,
          }
          return accumulator
        },
        {}
  ...
  const clientItemCategoryStyle: string =
    "flex items-center justify-center text-color_3 rounded-md h-10 text-xl my-2 first:mt-0"
  const clientItemContent: string =
    "grid grid-cols-1 gap-3 min-h-[50px] w-full mb-4"
  const clientItemContentTitle: string =
    "w-full h-full bg-color_6 text-color-4 rounded text-base capitalize p-2 truncate"
  ...
  clientKeys.forEach((key: ClientKeys) => {
    if (key === "id") return
      ...
      clientData[key].forEach((property: ClientPropertyI) => {
        const convertedProperty: PropertyItemRemappedT =
          convertedPropertyData && convertedPropertyData[property.id]
        if (
          convertedProperty &&
          convertedProperty.type === PropertyTypeE.checkbox
        ) {
          basicInfoElements.push(
            <div key={property.id} className={clientItemContent}>
              <span
                title={convertedProperty.label}
                className={clientItemContentTitle}
              >
                {convertedProperty.label}
              </span>
      ...
      propertyElements.push(
        <div key={key} className={clientItemContent}>
          <span title={key} className={clientItemContentTitle}>
            {key}
          </span>
  ...
  return (
    <div data-testid="client-list-card" className={clientItemStyle}>
      <h3 className={clientItemCategoryStyle}>
        {text["title-client-card-basic-info"]}
      </h3>
      {propertyElements}
      {!!basicInfoElements.length && (
        <>
          <h3 className={clientItemCategoryStyle}>
            {text["title-client-card-properties"]}
          </h3>
          {basicInfoElements}
        </>
      )}
